...
write.csv(book.df, file = "./output-files/lemmas-fulltable.csv")
head(book.df, 20)
```
...
```{r UniqueLemmas}
# 'dplyr' package needed for the following operation
...
uniqueLemmas.df <- book.df %>%
    ...
    ungroup() %>%
    mutate(freq = n /sum(n))
write.csv(uniqueLemmas.df, file = "./output-files/unique-lemmas.csv")
```
...
# Output
write.csv(nounsLemmas.df, file = "./output-files/nouns-lemmas.csv")
